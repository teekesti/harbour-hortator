...
Sailfish OS interval/exercise timer app (`harbour-horator`), similar in purpose to GymBoss. Built with the Sailfish SDK (Qt/QML + C++ backend), packaged as RPM.
...
- Open `harbour-horator.pro` in Sailfish IDE / Qt Creator with the Sailfish SDK kit, or build with `sfdk build` from the SDK toolchain.
...
- **`ExerciseTimer`** (`exercisetimer.h/cpp`) — the central state machine and clock. Owns an `ExerciseListModel`, drives a `QTimer`-based tick loop (`timerEvent`, 100ms interval) that advances `currentRunningTime`/`totalRunningTime`, computes per-activity and overall progress, fires rep notifications (sound) at computed intervals, and advances to the next list item when an activity's duration is reached. Also handles the pre-start countdown (`startDelay`) and an end-of-activity warning countdown (`endWarningTime`), both driven through a shared `mCountdownTimer` + `countDown(int)` signal consumed by `SoundPlayer`. Settings (`startDelay`, `endWarningTime`) persist via `QSettings`. Exposed to QML as the `exerciseTimer` context property (set up in `harbour-horator.cpp`).
...
- QML types are registered once in `main()` (`harbour-horator.cpp`); `exerciseTimer` and `exerciseListModel` are injected as QML context properties rather than instantiated in QML.

### QML layer (`qml/`)

- `harbour-horator.qml` — `ApplicationWindow` entry point; `initialPage` is `FirstPage`.
...
- `harbour-horator.pro.user` is a local Qt Creator/SDK user-environment file, not meant to be committed.

## Agent skills

### Issue tracker

Issues live in GitHub Issues (`teekesti/harbour-horator`), managed via the `gh` CLI. External PRs are not a triage surface. See `docs/agents/issue-tracker.md`.
